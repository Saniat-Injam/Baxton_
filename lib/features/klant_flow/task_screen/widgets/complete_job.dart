...
class Completejob extends StatelessWidget {
  ...
  const Completejob({super.key, required this.jobscontroller});

  @override
  Widget build(BuildContext context) {
    return Obx(() {
      final completedJobs = jobscontroller.jobList
          .where((job) => job.status.toLowerCase() == 'completed' || job.status.toLowerCase() == 'voltooid')
          .toList();
      if (completedJobs.isEmpty) {
        return Text(
          "Geen voltooide diensten",
          style: getTextStyle(
            fontSize: 16,
